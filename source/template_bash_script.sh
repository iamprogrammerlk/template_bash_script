...
export APP_NAME="Template Bash Script"
export APP_SLUG="template_bash_script"
...
declare -A app_configuration=(
  ["app_url"]="https://iamprogrammer.lk/template_bash_script"
  ["app_license"]="WTFPL License"
  ["app_license_url"]="https://github.com/iamprogrammerlk/template_bash_script#WTFPL-1-ov-file"
...
if [ ! -f "$APP_ROOT/library_pretty_bash/source/config.sh" ]; then
  echo "Runtime Error : '/library_pretty_bash/source/config.sh' is require to run '$APP_NAME'." >&2
  echo ""
  exit 1
fi
. $APP_ROOT/library_pretty_bash/source/config.sh

if [ ! -f "$APP_ROOT/library_pretty_bash/source/utility.sh" ]; then
  echo "Runtime Error : '/library_pretty_bash/source/utility.sh' is require to run '$APP_NAME'." >&2
  echo ""
  exit 1
fi
. $APP_ROOT/library_pretty_bash/source/utility.sh

if [ ! -f "$APP_ROOT/library_pretty_bash/source/style.sh" ]; then
  echo "Runtime Error : '/library_pretty_bash/source/style.sh' is require to run '$APP_NAME'." >&2
  echo ""
  exit 1
fi
. $APP_ROOT/library_pretty_bash/source/style.sh

if [ ! -f "$APP_ROOT/library_pretty_bash/source/ui.sh" ]; then
  echo "Runtime Error : '/library_pretty_bash/source/ui.sh' is require to run '$APP_NAME'." >&2
  echo ""
  exit 1
fi
. $APP_ROOT/library_pretty_bash/source/ui.sh
